...
<head>
    ...
    <style>
        ...
        :root { --glass-bg: rgba(255, 255, 255, 0.08); --accent: #00d2ff; --premium: #ffd700; --text-main: #ffffff; --text-muted: #a0a0a0; }
        * { box-sizing: border-box; }
        body { margin: 0; font-family: 'Outfit', sans-serif; background: #0f0c29; color: white; height: 100vh; overflow: hidden; }
        /* ... Copia el resto de tu CSS aquí ... */
        
        /* Asegúrate de incluir los estilos de .movie-grid, .movie-poster, etc. */
        .movie-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 2rem; padding-bottom: 4rem; }
        .movie-poster { border-radius: 16px; overflow: hidden; position: relative; aspect-ratio: 2/3; cursor: pointer; transition: 0.4s; }
        .movie-poster img { width: 100%; height: 100%; object-fit: cover; }
        .movie-poster:hover { transform: scale(1.1) translateY(-10px); z-index: 10; box-shadow: 0 20px 40px rgba(0,0,0,0.5); border: 2px solid var(--accent); }
        .badge-premium { background: var(--premium); color: black; padding: 2px 6px; border-radius: 4px; font-weight: bold; font-size: 0.7rem; position: absolute; top: 10px; right: 10px; z-index: 5; }
    </style>
</head>
<body>

    <div id="dynamic-bg" style="position: fixed; top:0; left:0; width:100%; height:100%; z-index:-1; transition: 0.5s; opacity: 0.4;"></div>

    <section id="view-splash" style="position:absolute; width:100%; height:100%; background:black; z-index:9999; display:flex; justify-content:center; align-items:center; flex-direction:column;">
        <h1 style="font-family:'Playfair Display'; font-size:4rem; color:white;">LA BUTACA</h1>
        <div style="width:200px; height:2px; background:#333; margin-top:20px;"><div id="loader" style="width:0%; height:100%; background:#00d2ff; transition: width 1s;"></div></div>
    </section>

    <section id="view-home" style="display:none; width:100%; height:100%; display:flex;">
        
        <nav style="width:250px; background:rgba(0,0,0,0.5); backdrop-filter:blur(10px); padding:20px; display:flex; flex-direction:column;">
            <h2 style="font-family:'Playfair Display'; margin-bottom:40px;">LB</h2>
            <a href="#" style="color:white; text-decoration:none; margin-bottom:20px; font-weight:bold; color:#00d2ff;">Explorar</a>
